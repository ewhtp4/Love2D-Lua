...
function Board:init(x, y, level)
	...
	self.level = level
	self.matches = {}
	-- ADDED- table of tiles in play for quicker match checking
	self.colorCount = 6
	self.varietyCount = 5
	self.tilesInPlay = {}
...
-- ADDED - Added for cleaner tile creation
function Board:tileCreator(x_pos, y_pos)
	-- ADDED- limiting colors for easier matching
	...
	-- ADDED- variety is level dependent
	...
	-- ADDED - adding to table of colors and varieties in play
		...
		for x = 2, 8 do
			...
			if self.tiles[y][x].color == colorToMatch then
				...
				if matchNum >= 3 then
					...
					-- ADDED.3 - shiny matched clear whole row
		...
		for y = 2, 8 do
			if self.tiles[y][x].color == colorToMatch then
				...
				if matchNum >= 3 then
					...
					-- ADDED.3 - shiny matched clear whole column
...
function Board:removeMatches()
	for k, match in pairs(self.matches) do
		for k, tile in pairs(match) do
			-- ADDED.4 - removing count from table
...
-- ADDED - Cascades tiles downward for a nice animation
...
-- ADDED - Tiles drop to the bottom of where they can
...
-- ADDED - wanted for falling tile animation
...
function Board:getFallingTiles()
	...
	for x = 1, 8 do
		for y = 8, 1, -1 do
			...
			if not tile then
				-- ADDED.3 - factoring number of tiles into tween speed
