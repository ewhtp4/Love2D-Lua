...
function PlayState:init()
	...
	-- ADDED.4 - stores matches
	self.matches = {}

	-- ADDED.5 - Mouse member variable
	self.cursorX = 0
	self.cursorY = 0
...
function PlayState:update(dt)
	...
	-- ADDED.5 - update cursor positions and adjust to screen
	...
	if self.canInput and self:mouseInBounds() then
		...
		-- ADDED- if we've pressed mouse, to select or deselect a tile...
		if love.mouse.wasPressed(1) then

			-- ADDED - fixing bug allowing selection of tile outside of board
			...
			elseif math.abs(self.highlightedTile.gridX - x) + math.abs(self.highlightedTile.gridY - y) > 1 then
				...
				Timer.tween(0.1, {
					...
					-- ADDED.4 - no match causes error and swap back
					if match == false then
						-- Swap back
						Timer.tween(0.1, {
							[self.highlightedTile] = {x = newTile.x, y = newTile.y},
...
function PlayState:calculateMatches()
	...
	if self.matches then
		...
		for k, match in pairs(self.matches) do
			...
			-- ADDED.2 - each step of variety is 10 more to score
			for i, tile in pairs(match) do
				self.score = self.score + tile.variety * 10 - 10
			-- ADDED.1 - adding +1 to timer for each match
			end
			self.timer = self.timer + 1
		end
...
function PlayState:render()
	...
	-- ADDED - Draw cursor rect if mouse in bounds
