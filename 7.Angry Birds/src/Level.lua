--[[
    ...
    Author: Colton Ogden modified by Slavko Mihajlovic
...
function Level:init()
    ...
    -- ADDED - Can't split after collision, so keeping track of it
    ...
    function beginContact(a, b, coll)
        ...
        -- ADDED - checking for collision to disable splitting
...
function Level:update(dt)
    ...
    for k, player in pairs(self.players) do
        local xPosition, yPosition = player.body:getPosition()
        local xVelocity, yVelocity = player.body:getLinearVelocity()
        
        -- if we fired our alien to the left or it's almost done rolling, respawn
        -- ADDED - Check if all players stoped 
        if xPosition < 0 or (math.abs(xVelocity) + math.abs(yVelocity) < 5) then
            player.body:destroy()
    ...
    if self.launchMarker.launched then
        local xPosition, yPosition = self.launchMarker.alien.body:getPosition()
        local xVelocity, yVelocity = self.launchMarker.alien.body:getLinearVelocity()
        
        -- if we fired our alien to the left or it's almost done rolling, respawn
        -- ADDED - Check if all players stoped 
        if xPosition < 0 or (math.abs(xVelocity) + math.abs(yVelocity) < 5) and #self.players == 0 then
    ...
    -- ADDED - Split Logic
    if love.keyboard.isDown(PLAYER_SPLIT) and not self.split 
        ...
        local topAlien = Alien(self.world, 'round', playerX + 15, playerY - 40, 'Player', self.launchMarker.sprite)
        local botAlien = Alien(self.world, 'round', playerX - 15, playerY + 40, 'Player', self.launchMarker.sprite)
        ...
        topAlien.fixture:setRestitution(0.7)
        botAlien.fixture:setRestitution(0.7)
...
function Level:render()
    ...
    --     love.graphics.printf(tostring(self.split),0,0,VIRTUAL_WIDTH,'right')
    -- end
end