...
function AlienLaunchMarker:init(world)
    ...
    self.alien = nil
    self.sprites = {6, 7, 9, 10, 15}
    self.sprite = self.sprites[math.random(1, 5)]
end
...
function AlienLaunchMarker:update(dt)
    ...
    if not self.launched then
        ...
        elseif love.mouse.wasReleased(1) and self.aiming then
            ...
            self.alien = Alien(self.world, 'round', self.shiftedX, self.shiftedY, 'Player', self.sprite)

            -- apply the difference between current X,Y and base X,Y as launch vector impulse
            self.alien.body:setLinearVelocity((self.baseX - self.shiftedX) * 10, (self.baseY - self.shiftedY) * 10)

            -- make the alien pretty bouncy
            self.alien.fixture:setRestitution(0.7)
    ...
    if not self.launched then
        ...
        if self.aiming then
            ...
            for i = 1, 90 do
                ...
                love.graphics.setColor(255, 80/255, 255, (255 / 12) * i)
