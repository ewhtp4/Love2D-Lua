...
function Brick:hit(paddle, key)
    ...
    if self.isLocked and not key then
        self.powerup = Powerup(10, self, paddle)    
    elseif key and self.isLocked and self.inPlay then
        self.inPlay = false
    end    
    ...
    if not self.isLocked then
        ...
        if math.random(1,2) == 1 and not self.powerup then
            self.powerup = Powerup(9, self, paddle)
        end
    end        
